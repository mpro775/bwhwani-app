...
export const fetchAllDonors = async (filters?: { governorate?: string; bloodType?: string }) => {
  ...
  const res = await axiosInstance.get(`/blood/donors?${params.toString()}`);
  return res.data;
};
export const fetchBloodMessages = async (requestId: string) => {
  const headers = await getAuthHeaders();
  const res = await axiosInstance.get(`/blood/messages/${requestId}`, { headers });
  return res.data;
};
...
export const deleteBloodData = async () => {
  const headers = await getAuthHeaders();
  const res = await axiosInstance.patch("/users/blood-settings", {
    bloodType: null,
